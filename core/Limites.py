    ...
    def reflex(superior: list, inferior: list, individuo: np.array) -> np.array:
        ...
        for j in range(len(individuo)):
            if individuo_corregido[j] < inferior[j]:
                individuo_corregido[j] = inferior[j] + (inferior[j] - individuo_corregido[j]) % range_width[j]
            elif individuo_corregido[j] > superior[j]:
                individuo_corregido[j] = superior[j] - (individuo_corregido[j] - superior[j]) % range_width[j]
